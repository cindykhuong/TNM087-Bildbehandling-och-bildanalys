...
Mask = IR_Image>Lower & IR_Image<Higher; %
    
% Use the Mask to mask out the areas within ths selected IR-range in the RGB-images
% (for all 3 color channels)

RGB_Range(:,:,1) = RGB_Image(:,:,1).*Mask; %red 
RGB_Range(:,:,2) = RGB_Image(:,:,2).*Mask; %green 
RGB_Range(:,:,3) = RGB_Image(:,:,3).*Mask; %blue 
...
IR_Select(y-5:y+5, x-5:x+5, 1) = 1; %created square    
